...
func (t *TaskWorker) Start() {
	go func() {
		for {
			...
			//log.Println("t.TaskWorkerQueue receive from t.TaskChannel")
			select {
...
func StartTaskDispatcher(taskWorkerSize int) {
	...
	go func() {
		for {
			...
			case task := <-TaskQueue:
				go func() {
					//log.Println("we receive task from TaskQueue")
					taskChannel := <-TaskWorkerQueue
					//log.Println("we assign taskChannel from TaskWorkerQueue")
					taskChannel <- task
					//log.Println("we receive taskChannel from task")
				}()
			}
		}
	}()
